...
def player_choice(player_input):
    ...
    player_input_tuple = [option for option in options if option[0] == player_input][0]
    ...
    if player_input_tuple == computer_input:
        winner_label.config(text="Tie")
    elif (player_input_tuple[1] - computer_input[1]) % 3 == 1:
        player_score += 1
...
scissors_btn = Button(input_frame, text='Scissors', width=15, bd=0, bg='light blue', pady=5,
...
play_again_btn = Button(input_frame, text='Play Again', width=15, bd=0, bg='light grey', pady=5, command=play_again)
play_again_btn.grid(row=5, columnspan=4, pady=10)

game_window.geometry('700x350')
